 ...
 *
 * `@mongez/encryption` currently has no sibling-package source deps — its
 * only runtime dependency is `crypto-js` — but we keep the pattern so the
 * config stays uniform with the rest of the workspace.
 */
